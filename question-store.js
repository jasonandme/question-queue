((root) => {
  const STATUS_KEYS = ["pending", "inserted", "answered", "learned"];
  const SOURCE_TYPES = ["article", "chat", "page"];
  const DEFAULT_FILL_PREFIX = "请依次回答以下问题，并保留编号：";
  const MAX_TAGS = 20;
  const LIMITS = { id: 120, question: 4000, context: 20000, notes: 8000, title: 300, tag: 40, key: 400 };
  ...
  function sanitizeItem(raw) {
    ...
    const item = {
      id,
      question,
      status: STATUS_KEYS.includes(raw.status) ? raw.status : "pending",
